...
* The output of the code below is  a **crosswalk from coordinates to Zipcode** that is stored in our [github here](https://github.com/courtneyjdiamond/pedalstopatterns.github.io/blob/main/data/geocoding/citibike_latlong_zip.csv) and used in our [Data Sources page](../import_cleaning/data_import_clean.html). 
...
The following code loads and tidies the Citibike data, identically as described in [Data Sources](../../import_cleaning/data_import_clean.html).

```{r}
citibike <- 
  tibble(
    files = list.files("../../citibike_alt/"),
    path = str_c("../../citibike_alt/", files)
  ) |>
...
For `r lat_long |> nrow()` coordinates, zipcodes were identified for `r latlong_zip |> filter(!is.na(postcode)) |> nrow()`, and `r latlong_zip |> filter(is.na(postcode)) |> nrow()` are missing zipcodes. These are handles later in data cleaning in [Data Sources](../import_cleaning/data_import_clean.html), where we manually lookup the remaining zipcodes, which is feasible for such a small subset. The manually annotated coordinates are stored on our [github here](https://github.com/courtneyjdiamond/pedalstopatterns.github.io/blob/main/data/geocoding/manual_zipcodes.csv).

## <span style='color: blue;'>Merge to Citibike</span>

  &nbsp;
  
The below code is just a quick test and preview of how we bring Zipcode back onto Citibike data. This is done in full in [Data Sources](../import_cleaning/data_import_clean.html). 
