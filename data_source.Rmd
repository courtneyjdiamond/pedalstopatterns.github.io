...
```{r sdi import and clean, show_col_types = FALSE}
...
```{r import overweight data, show_col_types = FALSE}
...
```{r air quality, show_col_types = FALSE}
...
```{r import and tidy UFH42 pdf, show_col_types = FALSE}
...
1. First, we need to convert the Citibike location data from latitude/longitude to zip code.This was done using a package [tidygeocoder](https://jessecambon.github.io/tidygeocoder/). Here, we import the latitude/longitude to zipcode crosswalk we generated. The full code with details of how that was created [can be found here](data/geocoding/latlong_to_zip.html). 
