...
### 3. Health Implications
   ...
   b. Are there differences in Citibike stations per neighborhood or ridership that is related to obesity? Do NYers who are overweight use Citibike more or less?
   
   c. Can we build a model of Citibike utilization that integrates disparities, both social and environmental, and also health factors.
...
We began our analysis by focusing on an exploration of the Citibike geographic data. We made a table of the stations used and available per neighborhood.
...
Next we looked at user demographics. 

![](analysis_files/figure-html/user_demographics-1.png)
 &nbsp;
 
Next, we explored the relationship between Citibike geographic utilization and SDI and percentage overweight by neighborhood. 
...
Next, we looked at ridership and median SDI at the zipcode level. 
...
And ridership and percentage overweight at the zipcode level.
...
Finally, we performed an exploration of of the health variables individually by NYC location. [Check this out for a full view](Exploration.html), but here are some highlights.
...
![](Exploration_files/figure-html/unnamed-chunk-8-1.png)
...
To build upon our exploratory analyses above we decided to pursue GLM with two sets of outcome in mind. The first was utilization of Citibike in a geographic area, based on health factors in the form of Air quality, SDI, and overweight percentages. This utilized a linear model that at first took these three predictors separately into consideration. We then created an interaction model that included each of the three individual predictors, but added all possible two-way interactions, and a three-way interaction as we hypothesized that the health, environmental influences, and utilization of biking as a mode of transportation and leisure were related. 
