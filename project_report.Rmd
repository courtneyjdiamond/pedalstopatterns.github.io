...
For the above missing zipcodes, we manually looked up based on latitude, longitude, and station_name what the correct zipcode is. Upon manually exploration, these looked like some data quality issues with the coordinates, but based on the staton name we were able to identify the correct zipcode. We stored this work in a .csv on our [github here](https://github.com/courtneyjdiamond/pedalstopatterns.github.io/blob/main/data/geocoding/manual_zipcodes.csv). The code below implements filling in those missing zipcode values. 
...
When we specifically looked at the relationship between median SDI and Citi Bike station usage/availability by NYC location, there didn't seem to be too much discernible pattern. But the largest number of available stations, though, did appear in the lower median SDI areas (the areas with lowest social deprivation, ie more privileged areas). We did not see much of a pattern when we looked at this variable by percentage overweight.

We next looked at how ridership is impacted by median SDI and percentage overweight. We found a clear downward trend in ridership as median SDI increased, but also low ridership at the lowest end of median SDI (ie more privileged zip codes). Perhaps this speaks to alternative modes of transportation that people in these zip codes take at both ends of the SDI spectrum. Unlike median SDI, there didn't seem to be a dip in ridership for the low range of overweight percentage by zip code. There did appear to be more consistent decreases in ridership as overweight percentage increases, which is not surprising. It is likely that that people who are overweight are less fit and use Citi bikes less.
